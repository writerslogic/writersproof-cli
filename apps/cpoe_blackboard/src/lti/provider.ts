...
// WritersProof Blackboard Learn — LTI 1.3 OIDC launch handler
...
// Blackboard-specific LTI custom claims
const CLAIM_BB_COURSE_ID = "https://blackboard.com/lti/claim/courseId";
const CLAIM_BB_CONTENT_ID = "https://blackboard.com/lti/claim/contentId";
...
export function createLtiRouter(
	bbPlatformUrl: string,
	bbClientId: string,
	toolHostUrl: string,
): Router {
	const router = Router();

	// Blackboard JWKS URI for validating id_token signatures.
	// Format: https://{institution}.blackboard.com/api/v1/management/applications/{appId}/jwks
	// The appId equals the client_id registered with Blackboard.
	const bbJwksUrl = new URL(
		`/api/v1/management/applications/${encodeURIComponent(bbClientId)}/jwks`,
		bbPlatformUrl,
	);
	const JWKS = createRemoteJWKSet(bbJwksUrl);

	// GET /lti/login — OIDC initiation (step 1)
	router.get("/login", (req: Request, res: Response) => {
		...
		if (client_id && client_id !== bbClientId) {
		...
		const authUrl = new URL("/api/v1/gateway/oidcauth", bbPlatformUrl);
		authUrl.searchParams.set("response_type", "id_token");
		authUrl.searchParams.set("response_mode", "form_post");
		authUrl.searchParams.set("scope", "openid");
		authUrl.searchParams.set("client_id", bbClientId);
	...
	// POST /lti/launch — id_token validation (step 2)
	router.post("/launch", async (req: Request, res: Response) => {
		try {
			...
			// Blackboard-specific course/content IDs take priority over standard context claim.
			const courseId =
				(claims[CLAIM_BB_COURSE_ID] as string | undefined) ??
				context["id"] ??
				"";
			const contentId =
				(claims[CLAIM_BB_CONTENT_ID] as string | undefined) ?? "";
	...
	// POST /lti/deep-linking — return a content item to Blackboard
	router.post("/deep-linking", async (req: Request, res: Response) => {
		try {
			...
			const jwt = await new SignJWT({
				...
				.setIssuer(bbClientId)
				.setAudience(bbPlatformUrl)
				.setIssuedAt(now)
...
<script>document.getElementById('dl').submit();</script>
	...
	// GET /lti/jwks — expose tool's public key
...
function renderToolHtml(ctx: LtiLaunchContext, hostUrl: string): string {
	...
	const contentIdSafe = escapeHtml(ctx.contentId);
...
<body>
  <div class="card">
    ...
    <p>Content ID: <code>${contentIdSafe}</code></p>
