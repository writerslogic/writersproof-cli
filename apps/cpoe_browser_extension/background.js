			...
			if (msg && typeof msg.protocol_version === "number") {
				if (msg.protocol_version < MIN_NATIVE_PROTOCOL_VERSION) {
					...
					broadcastToPopup({
						type: "error",
						message:
							"Desktop app is outdated. Please update for full attestation.",
					});
					return;
				}
	...
	if (secureChannel && secureChannel.handshakeComplete) {
		secureChannel
			...
			.catch((err) => {
				broadcastToPopup({
					type: "error",
					message: "Secure channel encrypt failed",
				});
			});
		return;
	}
...
function handleNativeMessage(message) {
	...
	switch (message.type) {
		...
		case "hello_accept":
			if (
				secureChannel &&
				!secureChannel.handshakeComplete &&
				secureChannel._handshakeResolve
			) {
				secureChannel
					.handleHelloAccept(message, (msg) =>
			...
			if (secureChannel && secureChannel.handshakeComplete) {
				secureChannel
					...
					.catch((err) => {
						...
						const isDesync =
							msg.includes("Ratchet count desync") ||
						...
						if (
							...
							console.error(
								"[CPoE] Secure channel re-handshake limit reached; falling back to plaintext",
							);
							secureChannel.destroy();
							secureChannel = null;
							rehandshakeAttempts = 0;
		...
		case "session_started":
			...
			if (message.session_id && _pendingResumeUrl) {
				chrome.storage.local.get(["_nativePendingResume"], (r) => {
					if (r._nativePendingResume) {
						chrome.storage.local.set({
							_nativePendingResume: {
								...r._nativePendingResume,
			...
			broadcastToPopup({
				...
				device_public_key: message.device_public_key,
		...
		case "status":
			broadcastToPopup({
				...
				checkpoint_count: message.checkpoint_count,
				tracked_files: message.tracked_files,
			});
			break;
...
function loadCustomDomains() {
	chrome.storage.local.get(["customDomains"], (result) => {
		customDomainList = (result.customDomains || []).filter(
...
function syncCustomContentScripts(domains) {
	if (!domains || domains.length === 0) {
		chrome.scripting
			.unregisterContentScripts({ ids: [CUSTOM_SCRIPT_ID] })
			.catch(() => {});
		return;
	}
		...
		const ms =
			typeof raw === "number"
				? Math.max(
						...
						Math.min(MAX_CHECKPOINT_INTERVAL_MS, raw * 1000),
					)
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	...
	switch (message.action) {
		case "start_witnessing":
			{
				...
				const startEditorType =
					typeof message.editorType === "string" && message.editorType
						? message.editorType
						: null;
				sendNativeMessage({
					type: "start_session",
					...
					timer_resolution_ms:
						typeof message.timerResolution === "number"
							? message.timerResolution
							: 0,
		...
		case "content_changed":
			{
				...
				ready
					.then(() => {
						...
						console.warn(
							"[CPoE] Checkpoint sent without commitment (genesis failed or session nonce missing)",
						);
						sendNativeMessage(checkpointMsg);
						sendResponse({
							ok: true,
					...
					.catch((err) => {
						console.error(
							"[CPoE] Commitment computation failed:",
							err?.message || err,
						);
		...
		case "browser_keystroke_batch":
			if (message.keystrokes && Array.isArray(message.keystrokes)) {
				sendNativeMessage({
					type: "browser_keystroke_batch",
					keystrokes: message.keystrokes.map((k) => [k.t, k.k, k.c]),
					tab_id: sender.tab ? sender.tab.id : 0,
				});
			}
			sendResponse({ ok: true });
			break;
		...
		case "export_evidence":
			sendResponse({
				ok: false,
				error: "Export not available in native mode; use the desktop app",
			});
			break;
		...
		case "content_changed":
			{
				if (!standaloneSessionId) {
					sendResponse({
						ok: false,
						error: "No active standalone session",
					});
					break;
				}
	...
	return new Promise((resolve, reject) => {
		...
		req.onupgradeneeded = () => {
			...
			if (!db.objectStoreNames.contains(ATTEST_STORE_NAME)) {
				db.createObjectStore(ATTEST_STORE_NAME, {
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
	...
	const tierDescs = {
		...
		corroborated:
			"Authorship attestation, sentinel active during authoring.",
