	...
	return new Promise((resolve, reject) => {
		...
		req.onupgradeneeded = (e) => {
			...
			if (oldVersion < 1) {
				...
				cpStore.createIndex("sessionId", "sessionId", {
					unique: false,
				});
			}
			...
			if (oldVersion < 2) {
				if (!d.objectStoreNames.contains(STORE_JITTER)) {
					...
					jStore.createIndex("sessionId", "sessionId", {
						unique: false,
					});
				}
			}
...
async function standaloneCheckpoint(
	sessionId,
	contentHash,
	charCount,
	delta,
	...
	if (
		ordinal > 1 &&
		now - session.lastCheckpointAt < MIN_CHECKPOINT_INTERVAL_MS
	) {
		return {
			type: "error",
			code: "RATE_LIMITED",
	...
	if (ordinal % QUOTA_CHECK_INTERVAL === 0) {
		const quota = await checkStorageQuota();
		if (quota && quota.remaining < QUOTA_WARNING_BYTES) {
...
async function standaloneExportEvidence(sessionId) {
	...
	const session = await storeGet(tx.objectStore(STORE_SESSIONS), sessionId);
	...
	const allJitterRecords = await storeGetAllByIndex(
	...
	const evidence = {
		...
		aiCopyEvents: aiCopyEvents.map((ev) => ({
			timestamp: ev.timestamp,
			source: ev.source,
			charCount: ev.charCount || 0,
		})),
		exportSeal: null,
	};
	...
	const sealInput = JSON.stringify({
		...
		aiCopyEvents: evidence.aiCopyEvents,
...
async function checkStorageQuota() {
	...
	try {
		...
		return { remaining, quota: est.quota || 0, usage: est.usage || 0 };
	} catch (_) {
		return null;
	}
}
