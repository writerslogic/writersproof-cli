...
const SECURE_CHANNEL_PROTOCOL_VERSION = 2;
...
class SecureChannel {
	...
	async performHandshake(sendRaw) {
		...
		return new Promise((resolve, reject) => {
			...
			sendRaw({
				type: "hello",
				protocol_version: SECURE_CHANNEL_PROTOCOL_VERSION,
	...
	async handleHelloAccept(message, sendRaw) {
		...
		try {
			...
			if (
				typeof message.server_pubkey !== "string" ||
				message.server_pubkey.length === 0
			) {
				throw new Error(
					"Missing or empty server_pubkey in hello_accept",
				);
			}
			...
			if (serverPubKeyBytes[0] !== 0x04) {
				throw new Error(
					"Server pubkey is not uncompressed SEC1 format (missing 0x04 prefix)",
				);
			}
			...
			if (
				typeof message.confirm !== "string" ||
				message.confirm.length === 0
			) {
				throw new Error(
					"hello_accept: missing or invalid 'confirm' field",
				);
			}
			...
			if (!constantTimeEqual(confirmPlaintext, expectedConfirm)) {
				throw new Error(
					"Key confirmation failed: server derived different key",
				);
			}
	...
	async decryptRaw(data) {
		if (data.length < 36) {
			throw new Error(
				`Encrypted message too short: ${data.length} bytes`,
			);
		}
	...
	async decrypt(envelope) {
		...
		if (!ALLOWED_NATIVE_MSG_TYPES.has(parsed.type)) {
			throw new Error(
				`Decrypted message has disallowed type: ${parsed.type}`,
			);
		}
	...
	async computeJitterHash(intervals) {
		if (!Array.isArray(intervals) || intervals.length === 0) {
			throw new Error(
				"computeJitterHash: intervals must be a non-empty array",
			);
		}
		for (let i = 0; i < intervals.length; i++) {
			if (!Number.isFinite(intervals[i]) || intervals[i] < 0) {
				throw new Error(
					`computeJitterHash: intervals[${i}] is not a finite non-negative number`,
				);
			}
		}
	...
	async computeCommitment(sessionId, ordinal, contentHash, timestamp) {
		if (
			...
			throw new Error(
				"computeCommitment: sessionId must be a non-empty string up to 256 chars",
			);
		}
		if (!Number.isInteger(ordinal) || ordinal < 0) {
			throw new Error(
				`computeCommitment: ordinal must be a non-negative integer, got ${ordinal}`,
			);
		}
		if (
			typeof contentHash !== "string" ||
			contentHash.length === 0 ||
			contentHash.length > 256
		) {
			throw new Error(
				"computeCommitment: contentHash must be a non-empty string up to 256 chars",
			);
		}
		if (!Number.isFinite(timestamp) || timestamp < 0) {
			throw new Error(
				`computeCommitment: timestamp must be a non-negative finite number, got ${timestamp}`,
			);
		}
