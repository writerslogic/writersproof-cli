#!/bin/bash
# Build Debian package for writerslogic
...
sed -i "s/^writerslogic (.*)/writerslogic (${VERSION}-1)/" "${SOURCE_DIR}/debian/changelog"
...
tar czf "../writerslogic_${VERSION}.orig.tar.gz" -C "${BUILD_DIR}" "writerslogic-${VERSION}"
