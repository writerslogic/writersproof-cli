...
/**
 * Hashes arbitrary text content using SHA-256, producing a hex digest.
 * Used for snapshot fingerprinting before sending to the WritersProof API.
 */
	...
	private async request<T>(
		...
		for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
			...
			try {
				...
				// 429 — respect Retry-After, then retry regardless of attempt count
				if (resp.status === 429) {
