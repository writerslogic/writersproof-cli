...
#[cfg_attr(feature = "ffi", uniffi::export)]
pub fn ffi_verify_fingerprint_match(profile_id: String) -> FfiFingerprintVerification {
    catch_ffi_panic!(
    ...
    catch_ffi_panic!(None, {
        with_manager(|mgr| {
            Ok(mgr
                .canonical_profile
                .as_ref()
                .map(|fp| serde_json::to_string_pretty(fp).unwrap_or_default()))
        })
        .unwrap_or(None)
    })
}
