...
pub(crate) fn build_forensic_breakdown(
    ...
    ForensicBreakdown {
        ...
        snr_db: metrics.snr.as_ref().map(|s| s.snr_db).filter(|v| v.is_finite()),
        snr_flagged: metrics.snr.as_ref().is_some_and(|s| s.flagged),
        lyapunov_exponent: metrics.lyapunov.as_ref().map(|l| l.exponent).filter(|v| v.is_finite()),
        lyapunov_flagged: metrics.lyapunov.as_ref().is_some_and(|l| l.flagged),
        iki_compression_ratio: metrics.iki_compression.as_ref().map(|i| i.ratio).filter(|v| v.is_finite()),
        iki_compression_flagged: metrics.iki_compression.as_ref().is_some_and(|i| i.flagged),
        forgery_difficulty: metrics.forgery_cost.as_ref().map(|f| f.overall_difficulty).filter(|v| v.is_finite()),
        forgery_tier: metrics.forgery_cost.as_ref().map(|f| f.tier.to_string()),
        forgery_time_sec: metrics.forgery_cost.as_ref().map(|f| f.estimated_forge_time_sec).filter(|v| v.is_finite()),
        fatigue_warmup_pct: metrics.fatigue_trajectory.as_ref().map(|f| f.warmup_fraction).filter(|v| v.is_finite()),
        fatigue_plateau_pct: metrics.fatigue_trajectory.as_ref().map(|f| f.plateau_fraction).filter(|v| v.is_finite()),
        fatigue_pct: metrics.fatigue_trajectory.as_ref().map(|f| f.fatigue_fraction).filter(|v| v.is_finite()),
        fatigue_slope: metrics.fatigue_trajectory.as_ref().map(|f| f.fatigue_slope_iki_per_kstroke).filter(|v| v.is_finite()),
        cross_modal_score: metrics.cross_modal.as_ref().map(|cm| cm.score).filter(|v| v.is_finite()),
        cross_modal_verdict: metrics.cross_modal.as_ref().map(|cm| cm.verdict.to_string()),
        transcription_suspicious: metrics.transcription_suspicion.as_ref().is_some_and(|t| t.is_suspicious),
        repair_recent_pct: metrics.repair_locality.as_ref().map(|r| r.recent_repair_pct).filter(|v| v.is_finite()),
        repair_distant_pct: metrics.repair_locality.as_ref().map(|r| r.distant_repair_pct).filter(|v| v.is_finite()),
        cognitive_load_score: metrics.cognitive_load.as_ref().map(|cl| cl.composite_score).filter(|v| v.is_finite()),
        revision_topology_score: metrics.revision_topology.as_ref().map(|rt| rt.composite_score).filter(|v| v.is_finite()),
        detour_ratio: metrics.revision_topology.as_ref().map(|rt| rt.detour_ratio).filter(|v| v.is_finite()),
        leading_edge_divergence: metrics.revision_topology.as_ref().map(|rt| rt.leading_edge_divergence).filter(|v| v.is_finite()),
        insertion_point_entropy: metrics.revision_topology.as_ref().map(|rt| rt.insertion_point_entropy).filter(|v| v.is_finite()),
        error_ecology_score: metrics.error_ecology.as_ref().map(|ee| ee.composite_score).filter(|v| v.is_finite()),
        likelihood_p_cognitive: metrics.likelihood_model.as_ref().map(|lm| lm.session_p_cognitive).filter(|v| v.is_finite()),
