...
/// Extract plain document text for linguistic forensic analysis.
///
/// Mirrors `extract_word_count`'s format coverage for the common writing
/// formats so the report builder can feed real prose into cognitive-linguistic
/// coupling, AI-fluency, and language scoring. RTF control words are stripped.
/// Binary/zip formats (docx, odt) and unsupported extensions yield `None`.
pub(crate) fn extract_plain_text(path: &Path) -> Option<String> {
    let ext = path.extension().and_then(|e| e.to_str())?.to_lowercase();
    ...
    match ext.as_str() {
        "txt" | "md" | "markdown" | "text" | "fountain" | "org" | "rst" | "adoc" | "asciidoc"
        | "tex" | "latex" => std::fs::read_to_string(path).ok(),
