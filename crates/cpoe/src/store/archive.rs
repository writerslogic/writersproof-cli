...
impl SecureStore {
    ...
    pub fn archive_old_events(
        ...
        let first_remaining: Option<Vec<u8>> = self
            .conn
            .query_row(
        ...
        // Create the archive database with the same schema.
        self.create_archive_db(&archive_path, cutoff_ns, archive_count, &chain_link_hash)?;

        // Delete archived events from the active database.
        let tx = self.conn.transaction()?;
        let deleted: u64 = tx.execute(
            "DELETE FROM secure_events WHERE timestamp_ns < ?",
            params![cutoff_ns],
        )? as u64;

        if deleted != archive_count {
            // Rollback implicit on drop; also remove the archive file.
            drop(tx);
        ...
        // Update the integrity record: new event_count = old - archived.
        let (_old_count, _last_verified_seq, old_chain_hash): (i64, i64, Vec<u8>) = tx.query_row(
            "SELECT event_count, last_verified_sequence, chain_hash FROM integrity WHERE id = 1",
            [],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
        )?;

        // Set last_verified_sequence to the last remaining event's ID so the
        // verifier treats all remaining events as already verified. We validated
        // the full chain during this archival operation.
        //
        // The verifier checks: last_verified_seq + newly_scanned == event_count.
        // After archival, remaining IDs are sparse (e.g., 9,10,11,12 after deleting 1-8).
        // Set event_count = last_verified_seq = max(id) to maintain the invariant
        // since the verifier will find 0 new events (all pre-verified).
        let new_last_verified_seq: i64 = tx
            .query_row(
                "SELECT COALESCE(MAX(id), 0) FROM secure_events",
                [],
                |row| row.get(0),
            )?;
        ...
        tx.execute(
            "UPDATE integrity SET event_count = ?, last_verified_sequence = ?, \
             last_verified = ?, hmac = ? WHERE id = 1",
            params![
                new_count,
                new_last_verified_seq,
    ...
    /// Create an archive database and copy qualifying events into it.
    fn create_archive_db(
        &self,
        archive_path: &Path,
        cutoff_ns: i64,
        expected_count: u64,
        chain_link_hash: &[u8; 32],
    ) -> anyhow::Result<()> {
        ...
        // Create the same schema as the active DB for secure_events and integrity.
        archive_conn.execute_batch(
            ...
            CREATE INDEX IF NOT EXISTS idx_archive_events_timestamp ON secure_events(timestamp_ns);
            CREATE INDEX IF NOT EXISTS idx_archive_events_file ON secure_events(file_path, timestamp_ns);
        ...
        archive_conn.execute(
            "INSERT INTO archive_metadata (key, value) VALUES ('created_at', ?)",
            params![now_str],
        )?;
        archive_conn.execute(
            "INSERT INTO archive_metadata (key, value) VALUES ('cutoff_ns', ?)",
            params![cutoff_ns.to_string()],
        )?;
        archive_conn.execute(
            "INSERT INTO archive_metadata (key, value) VALUES ('chain_link_hash', ?)",
            params![hex::encode(chain_link_hash)],
        )?;

        // Copy events from active DB to archive via ATTACH.
        // We use the file path of the active DB to attach it.
        let active_db_path = self
            .conn
            .query_row("PRAGMA database_list", [], |row| row.get::<_, String>(2))?;

        archive_conn.execute(
            "ATTACH DATABASE ? AS active_db",
            params![active_db_path],
        )?;

        let inserted = archive_conn.execute(
            "INSERT INTO main.secure_events
                (device_id, machine_id, timestamp_ns, file_path, content_hash, file_size,
                 size_delta, previous_hash, event_hash, hmac, context_type, context_note,
                 vdf_input, vdf_output, vdf_iterations, forensic_score, is_paste,
                 hardware_counter, input_method, lamport_signature, lamport_pubkey_fingerprint,
                 challenge_nonce, hw_cosign_signature, hw_cosign_pubkey,
                 hw_cosign_salt_commitment, hw_cosign_chain_index, hw_cosign_entangled_hash,
                 hw_cosign_entropy_digest, hw_cosign_entropy_bytes, posme_proof, semantic_summary)
             SELECT device_id, machine_id, timestamp_ns, file_path, content_hash, file_size,
                 size_delta, previous_hash, event_hash, hmac, context_type, context_note,
                 vdf_input, vdf_output, vdf_iterations, forensic_score, is_paste,
                 hardware_counter, input_method, lamport_signature, lamport_pubkey_fingerprint,
                 challenge_nonce, hw_cosign_signature, hw_cosign_pubkey,
                 hw_cosign_salt_commitment, hw_cosign_chain_index, hw_cosign_entangled_hash,
                 hw_cosign_entropy_digest, hw_cosign_entropy_bytes, posme_proof, semantic_summary
             FROM active_db.secure_events
             WHERE timestamp_ns < ?
             ORDER BY id ASC",
            params![cutoff_ns],
        )?;

        archive_conn.execute("DETACH DATABASE active_db", [])?;

        if inserted as u64 != expected_count {
            let _ = std::fs::remove_file(archive_path);
            anyhow::bail!(
        ...
        // Create integrity record for the archive.
        ...
        archive_conn.execute(
            "INSERT INTO integrity (id, chain_hash, event_count, last_verified, last_verified_sequence, hmac)
             VALUES (1, ?, ?, ?, 0, ?)",
            params![
                &chain_link_hash[..],
                inserted as i64,
        ...
        // Switch archive to WAL checkpoint and then DELETE journal mode for read-only use.
        archive_conn.execute_batch("PRAGMA wal_checkpoint(TRUNCATE); PRAGMA journal_mode=DELETE;")?;
    ...
    pub fn verify_archive_chain_link(
        ...
        let first_prev_hash: Option<Vec<u8>> = self
            .conn
            .query_row(
