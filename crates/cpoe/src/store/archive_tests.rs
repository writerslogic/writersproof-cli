...
fn test_archive_preserves_hmac_integrity() {
    ...
    let result = store
        .archive_old_events(&db_path, Some(90))
        .unwrap()
        .unwrap();
...
fn test_archive_hmac_tamper_detected() {
    ...
    let now_ns = chrono::Utc::now().timestamp_nanos_safe();
    let err =
        SecureStore::query_archive(&result.archive_path, &wrong_key, "/test/doc.txt", 0, now_ns);
...
fn test_archive_chain_continuity() {
    ...
    // Verify chain link.
    assert!(store
        .verify_archive_chain_link(&result.archive_path)
        .unwrap());
}
...
fn test_query_spanning_both_dbs() {
    ...
    let all = store
        .query_spanning(&db_path, "/test/doc.txt", 0, i64::MAX)
        .unwrap();
...
fn test_archive_read_only_permission() {
    ...
    #[cfg(unix)]
    {
        ...
        let perms = std::fs::metadata(&result.archive_path)
            .unwrap()
            .permissions();
...
fn test_query_spanning_empty_archive() {
    ...
    let events = store
        .query_spanning(&db_path, "/test/file.txt", 0, i64::MAX)
        .unwrap();
...
fn test_multiple_files_across_archive_boundary() {
    ...
    let a_events = store
        .query_spanning(&db_path, "/test/a.txt", 0, i64::MAX)
        .unwrap();
    assert_eq!(a_events.len(), 7);

    // Query spanning for file b: 3 old + 0 new = 3.
    let b_events = store
        .query_spanning(&db_path, "/test/b.txt", 0, i64::MAX)
        .unwrap();
